...
class GetFeatures:
   ...
   def generateTestFeatures(self,pBatch):
    ...
    if "translation" in pBatch and pBatch["translation"] =="yes" :
        ...
        pBatch["translationFile"]=re.sub(".txt$","",pBatch["segmentsFile"])+"-translation.txt"
        tr=Translate(pBatch["segmentsFile"],pBatch["translationFile"],pBatch["sourceLanguage"],pBatch["targetLanguage"])
   ...
   def generateTrainingFeatures(self):
    ...
    if "translation" in self.pDict and self.pDict["translation"] =="yes" :
        logger.info("Add Translation Features")
        fTranslations=re.sub(".txt$","",self.pDict["fCategory"])+"-translation.txt"
        tr=Translate(self.pDict["fCategory"],fTranslations,self.pDict["sourceLanguage"],self.pDict["targetLanguage"])
        tr.addFeatures(segmentFeaturesDict)
